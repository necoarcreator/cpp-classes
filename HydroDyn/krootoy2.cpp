...
krootoy::krootoy(string file_path) {
	int count_ = 20;
	vector <string> bobs = pokushai(file_path, count_);
	...
	rho_new = new Grid(N_x); // 

	e_new = new Grid(N_x); // 

	ei_new = new Grid(N_x); // 

	vb_new = new Grid(N_x + 1); // 

	xb_new = new Grid(N_x + 1); // 
}
...
vector <string> krootoy::pokushai (string file_path, int count_ = 19) {
	ifstream file(file_path);
	vector <string> a(count_);
	if(file.is_open())
	{
...
void krootoy::solverCrestNotDiv(string file_path, string format = ".csv", char _delim = '/t',
				size_t recordFreq = 5, int test = 1, bool art_viscos = true) {
...
void krootoy::solverCrestDiv(string file_path, string format = ".csv", char _delim = '/t',
					size_t recordFreq = 5, int test = 1, bool art_viscos = true) {
