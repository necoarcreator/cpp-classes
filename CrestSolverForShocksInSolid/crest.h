...
class crest: public krootoy{
public:
	
	crest (string file_path);
	~crest();
	
	void writeResults (string file_path, string format, char _delim, double _time);
	
	void solverCrestNotDiv(string file_path, string format, char _delim,
				size_t recordFreq, int test, int problem_type, int v_0);
				
	void piston_no_wall(int v_0);
	void piston_wall(int v_0);
