...
int main(int argc, char** argv) {
	...
	for (ptrdiff_t i = 0; i < 11; ++i) {
		...
		u_theor[i] += 4 * u_0 / M_PI * exp(- k * M_PI * M_PI * (2 * m + 1) * (2 * m + 1) *
												(T / 10) / L / L) / (2 * m + 1) * sin(M_PI * (2 * m + 1) * x / L);
		}
	...
	while (time < 2 * T_stop && iter < max_iter) {
		...
		if (rank % 2 != 0) {
			if (rank > 0)
			{
				MPI_Send(&up[0], 1, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD); //0
				MPI_Recv(&u_left, 1, MPI_DOUBLE, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, &Status); //1
			}
			if (rank < size - 1)
			{
				MPI_Send(&up[chunk - 1], 1, MPI_DOUBLE, rank + 1, 2, MPI_COMM_WORLD); //2
				MPI_Recv(&u_right, 1, MPI_DOUBLE, rank + 1, MPI_ANY_TAG, MPI_COMM_WORLD, &Status); //3
			...
			if (rank < size - 1) {
				...
				MPI_Recv(&u_right, 1, MPI_DOUBLE, rank + 1, MPI_ANY_TAG, MPI_COMM_WORLD, &Status); //0
				MPI_Send(&up[chunk - 1], 1, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD); //1
				
			}
			if (rank > 0) {
				
				MPI_Recv(&u_left, 1, MPI_DOUBLE, rank - 1, MPI_ANY_TAG, MPI_COMM_WORLD, &Status); //2
				MPI_Send(&up[0], 1, MPI_DOUBLE, rank - 1, 3, MPI_COMM_WORLD); //3
